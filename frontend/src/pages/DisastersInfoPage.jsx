...
import RescueBot from "../components/RescueBotModal"; // Assuming this component exists and is correctly imported

// Main App Component
const DisastersInfoPage = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedDisaster, setSelectedDisaster] = useState(null);
  const [checkedItems, setCheckedItems] = useState({});
  const [showRescueBot, setShowRescueBot] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br mt-[5rem] from-blue-50 to-indigo-100">
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            ...
            <div className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white p-4 rounded-xl">
              ...
              <div className="text-sm opacity-90">RescueBot Support</div>
            </div>
      ...
      {/* RescueBot Floating Button */}
      <button
        onClick={() => {
            console.log("[CLICK BOT] Opening RescueBot modal.");
            setShowRescueBot(true);
        }}
        className="fixed bottom-6 right-6 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 z-40"
        aria-label="Open Rescue Bot" // Accessibility
      >
        <MessageCircle className="w-6 h-6" />
      </button>

      {showRescueBot && ( // Ensure RescueBot is only mounted when needed
         <RescueBot
            isOpen={showRescueBot}
            onClose={() => {
                console.log("[RescueBot] Closing modal.");
                setShowRescueBot(false);
            }}
          />
      )}
    </div>
...
export default DisastersInfoPage;